...
public class ArcherBattleState : EnemyState
{
    ...
    public override void Update()
    {
        ...
        if (enemy.IsPlayerDetected())
        {
            ...
            if (enemy.IsPlayerDetected().distance < enemy.safeDistance && enemy.facingDir != player.GetComponent<Player>().facingDir)
            {
                if (CanJump()) stateMachine.ChangeState(enemy.jumpState);
            }
    ...
    {
        if (player.position.x > enemy.transform.position.x && enemy.facingDir == -1) enemy.Flip();
        else if (player.position.x < enemy.transform.position.x && enemy.facingDir == 1) enemy.Flip();
    }
    ...
    {
        if (enemy.GroundBehind() == false || enemy.WallBehind() == true) return false;
