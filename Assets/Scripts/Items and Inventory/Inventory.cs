...
public class Inventory : MonoBehaviour, ISaveManager
{
    ...
    private void AddStartingItems()
    {
        ...
        {
            if (startingItems[i] != null) AddItem(startingItems[i]);
        }
    }
    ...
    private void UpdateSlotUI()
    {
        for (int i = 0; i < equipmentSlot.Length; i++)
        {
            foreach (KeyValuePair<ItemData_Equipment, InventoryItem> item in equipmentDictionary)
            {
                if (item.Key.equipmentType == equipmentSlot[i].slotType) equipmentSlot[i].UpdateSlot(item.Value);
            }
        }
    ...
    private List<ItemData> GetItemDataBase()
    {
        ...
        string[] assetNames = AssetDatabase.FindAssets("", new[] { "Assets/ItemData/Items" });
